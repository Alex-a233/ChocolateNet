...
class DDualGCN(nn.Module):
    ...
    def forward(self, feat):  # (16, 32, 44, 44)
        ...
        x_sqz = to_matrix(x_sqz)  # (16, 4, 44*44)

        b = self.theta(b)  # (16, 8, 44, 44)
        b = to_matrix(b)  # (16, 8, 44*44)
...
class SpatialGCN(nn.Module):
    ...
    def forward(self, x):  # (16, 32, 4, 4)
        ...
        am = torch.bmm(node_q, node_v)  # (16, 16, 16)
        am = self.softmax(am)  # (16, 16, 16)
        am = torch.bmm(node_k, am)  # (16, 4*4, 16)
        am = am.transpose(1, 2).contiguous()  # (16, 16, 4*4)
        amw = self.conv_wg(am)  # (16, 16, 4*4)
        amw = self.bn_wg(amw)
        amw = amw.view(b, c, h, -1)  # (16, 16, 4, 4)

        out = self.gamma * self.out(amw) + x  # (16, 32, 4, 4)
        return out
